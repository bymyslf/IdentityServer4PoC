...
var settings = {
    ...
    silent_redirect_uri:'http://localhost:5003/user-manager-sample-silent.html',
...
function startSigninMainWindowDiffCallbackPage() {
    mgr.signinRedirect({state:'some data', redirect_uri: 'http://localhost:5003/user-manager-sample-callback.html'}).then(function() {
        log("signinRedirect done");
    }).catch(function(err) {
        log(err);
    });
